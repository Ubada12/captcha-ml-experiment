...
|---|---|---|
| A | Dataset finalization | ✅ Complete — 3,035 / 338 / 32, zero leakage confirmed |
| B | `dataset.py` | ✅ Complete — verified by execution |
| C | `model.py` | ✅ Complete — verified by execution |
| D | Parameter audit | ✅ Complete — 14,336,075 total / 11,562,827 trainable / 2,773,248 frozen |
| E | CTC loss + greedy decoder | ✅ Complete |
| F1 | Engineering sanity test | ✅ Complete |
| F2 | Tiny-set learning test | ✅ Complete — 0% → 100% exact accuracy, epoch 1 → 85, verified from raw history |
| G | Full training | ✅ Complete — 42/50 epochs, best epoch 35, no train/val divergence |
| H | Best checkpoint + inference | ✅ Complete |
| I | Final evaluation | ✅ Complete — 31/32 full CAPTCHA, 189/192 digits |
| J | Experiment 4 vs. Experiment 6 | ✅ Complete — see table below |

All numbers above are independently verified against primary source files (raw JSON logs, re-executed model code, re-run inference against the untouched `test.jsonl`), not accepted from a written summary alone. Full detail in the companion `Experiment_6_Final_Report.md`.
...
**Verdict:** P3 and P4 — the exact positions Experiment 4 collapsed on — closed the gap with P1/P5/P6 almost entirely (P3 hit 100%; P4 rose from 43.75% to 96.88%). This is direct evidence the BiLSTM+CTC cross-position context mechanism did what it was designed to do, not just a headline-number improvement. The one remaining error (`633367→63367`, on the repeated-digit position) is a single CTC greedy-decoding collapse of an adjacent repeat, not a return of the positional weakness — see `Experiment_6_Final_Report.md` §6–7 for the full trace, including a methodological caveat about how the fixed-index accounting slightly overstates how many "positions" that one error touches.
